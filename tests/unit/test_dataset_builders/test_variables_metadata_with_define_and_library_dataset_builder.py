...
def test_build_combined_metadata(
    ...
    assert aeterm_row["define_vlm_present"] == True
    assert aeterm_row["define_vlm_item_count"] == 2
    assert aeterm_row["define_vlm_ccodes"] == []          # no codelists on either VLM item
    assert aeterm_row["define_vlm_has_codelist_any"] == False
    assert aeterm_row["define_vlm_has_codelist_all"] == False
    assert aeterm_row["define_vlm_ccode_missing_any"] == True  # both ccodes are empty
    assert aeterm_row["define_vlm_ccode_matches_library_any"] == False
    assert aeterm_row["define_vlm_ccode_matches_library_all"] == False
    ...
    for var in ["STUDYID", "USUBJID"]:
        row = result[result["variable_name"] == var].iloc[0]
        assert row["define_vlm_present"] == False
        assert row["define_vlm_item_count"] == 0
        assert row["define_vlm_ccodes"] == []
        assert row["define_vlm_has_codelist_any"] == False
        assert row["define_vlm_has_codelist_all"] == False
        assert row["define_vlm_ccode_missing_any"] == False
        assert row["define_vlm_ccode_matches_library_any"] == False
        assert row["define_vlm_ccode_matches_library_all"] == False
