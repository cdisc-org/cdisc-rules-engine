...
def test_get_detailed_data():
    ...
    errors = [
        [mock_validation_results[0].id, "AESTDY and DOMAIN are equal to test", "Error", "AE", "CDISC002", 1, 2, "AESTDY, DOMAIN", "test, test"],
        [mock_validation_results[0].id, "AESTDY and DOMAIN are equal to test", "Error", "AE", "CDISC003", 9, 10, "AESTDY, DOMAIN", "test, test"],
        [mock_validation_results[1].id, "TTVARs are wrong", "Warning", "TT", "CDISC002", 1, 2, "TTVAR1, TTVAR2", "test, test"],
    ]
...
def test_get_summary_data():
    ...
    errors = [
        ["AE", mock_validation_results[0].id, "AESTDY and DOMAIN are equal to test", "Error", 2],
...
def test_get_excel_export():
    with open(test_report_template, "rb") as f:
        report: ExcelReport = ExcelReport("test", mock_validation_results, 10.1, f.read())
