...
def process_rules(rules, library_data_service, packages):
    for rule in rules:
        try:
            ...
            print(
                (
                    f"Error while processing rule "
                    f"{rule.get('Core', rule.get('CoreId'))} {e}"
                )
            )
            continue
            ...
            print(
                (
                    f"Unexpected Error while processing rule "
                    f"{rule.get('Core', rule.get('CoreId'))} {e}"
                )
            )
            continue
...
if __name__ == "__main__":
    ...
    library_data_service = CDISCLibraryClient(
        config.getValue("LIBRARY_API_KEY"),
        base_api_url=config.getValue("LIBRARY_API_URL"),
    )
    ...
    for package in packages.values():
        print(
            (
                f"Package {package.package_name} has {len(package.rules.keys())}"
                f" published rules"
            )
        )
