...
[`cdisc-open-rules`](https://github.com/cdisc-org/cdisc-open-rules). If you want to:
...
Follow the [Development → Environment Setup](development.md#environment-setup) guide to
...
This installs the hooks into `.git/hooks/` so formatting and linting run automatically
on each commit.
...
This runs both unit and regression tests. All tests must pass before submitting a pull
request.
...
5. Open a pull request with a clear description of the change and the motivation behind
   it.

For larger changes or new features, consider opening a GitHub Discussion or issue first
...
Use [GitHub Issues](https://github.com/cdisc-org/cdisc-rules-engine/issues) to report
