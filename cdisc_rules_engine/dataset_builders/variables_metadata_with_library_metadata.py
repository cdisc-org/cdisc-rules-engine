...
class VariablesMetadataWithLibraryMetadataDatasetBuilder(BaseDatasetBuilder):
    def build(self):
        ...
        content_variables_metadata: DatasetInterface = self.data_service.get_variables_metadata(
        ...
        data["variable_has_empty_values"] = data.apply(
            lambda row: self.variable_has_null_values(row["variable_name"], dataset_contents),
            axis=1,
        )
        return data
