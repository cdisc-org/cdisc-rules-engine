...
def installed_whodrug_dictionaries(request) -> dict:
    ...
    whoddrug_path: str = f"{os.path.dirname(__file__)}/resources/dictionaries/whodrug"
    terms: dict = factory.install_terms(whoddrug_path)
    cache_service.add(whoddrug_path, terms)

    def delete_terms_from_cache():
        cache_service.clear(whoddrug_path)

    request.addfinalizer(delete_terms_from_cache)

    return {
        "whoddrug_path": whoddrug_path,
