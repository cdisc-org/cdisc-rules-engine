...
def test_get_code_hierarchies():
    data_service = LocalDataService.get_instance(cache_service=MagicMock(), config=MagicMock())
    factory = MedDRATermsFactory(data_service)
    terms = factory.install_terms(dictionary_path)
    expected_code_hierarchies = set(
        [f"SOC{i + 1}/HLGT{i + 1}/HLT{i + 1}/PT{i + 1}/LLT{i + 1}" for i in range(len(terms[TermTypes.LLT.value]))]
    )
