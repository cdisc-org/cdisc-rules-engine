...
from cdisc_rules_engine.services.reporting.csv_report import CsvReport
...
def test_get_report_services():
    ...
    for service in services:
        is_csv: bool = (
            isinstance(service, CsvReport)
            and not isinstance(service, ExcelReport)
            and not isinstance(service, JsonReport)
        )
        is_excel: bool = (
            isinstance(service, ExcelReport)
            and not isinstance(service, CsvReport)
            and not isinstance(service, JsonReport)
        )
        is_json: bool = (
            isinstance(service, JsonReport)
            and not isinstance(service, CsvReport)
            and not isinstance(service, ExcelReport)
        )
        assert is_csv or is_excel or is_json
