...
# Function to execute the TimeTestFunction
def TimeTestFunction(xpt_dir, rule_dir, total_calls):
    results = []  # List to store results for DataFrame

    # Collect all dataset files from XPT directory
    xpt_files = [os.path.join(xpt_dir, file) for file in os.listdir(xpt_dir) if file.endswith(".json")]
        ...
        for rule in rules:
            ...
            for num_call in range(total_calls):
                ...
                try:
                    ...
                    end_time = time.time()
                    # Parse logs from stdout for preprocessing time
            ...
            if len(time_taken) > 0:
                results.append({
                    ...
                    # Store preprocessing times as a string to ensure proper DataFrame handling
