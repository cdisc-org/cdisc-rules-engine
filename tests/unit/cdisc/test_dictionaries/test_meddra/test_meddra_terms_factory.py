...
def test_install():
    ...
    factory = AbstractTermsFactory(storage_service).get_service(DictionaryTypes.MEDDRA.value)
    ...
    for i, term in enumerate(dictionary[TermTypes.PT.value].values()):
        ...
        assert term.term_hierarchy == f"TESTSOC{i+1}/TESTHLGT{i+1}/TESTHLT{i+1}/TESTPT{i+1}"
    ...
    for i, term in enumerate(dictionary[TermTypes.LLT.value].values()):
        ...
        assert term.term_hierarchy == f"TESTSOC{i+1}/TESTHLGT{i+1}/TESTHLT{i+1}/TESTPT{i+1}/TESTLLT{i+1}"
