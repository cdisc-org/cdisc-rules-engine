...
def get_library_metadata_from_cache(args, sql=False) -> LibraryMetadataContainer:  # noqa
    ...
    if args.define_xml_path:
        ...
        if define_version.model_package == "define_2_1" and len(args.controlled_terminology_package) > 0:
            raise ValueError("Cannot use -ct controlled terminology package command with Define-XML2.1 submission")
        elif define_version.model_package == "define_2_0" and len(args.controlled_terminology_package) > 1:
            raise ValueError("Cannot provide multiple controlled terminology packages with Define-XML2.0 submission")
    ...
    with open(variables_codelist_file, "rb") as f:
        data = pickle.load(f)
        cache_key = get_standard_codelist_cache_key(args.standard, args.version.replace(".", "-"))
    ...
    ct_files = [file_name for file_name in cache_files if CodelistReader.FILENAME_PATTERN.match(file_name)]
...
def check_custom_standard(args):
    ...
    try:
        ...
        return False
    key = get_rules_cache_key(args.standard, args.version.replace(".", "-"), args.substandard)
...
def fill_cache_with_dictionaries(cache: CacheServiceInterface, args, data_service: DataServiceInterface) -> dict:
    """
...
def get_rules(args) -> List[dict]:
    return load_rules_from_local(args) if args.local_rules else load_rules_from_cache(args)
...
def load_specified_rules(rules_data, rule_ids, standard, version, standard_dict, substandard):
    ...
    for rule in rule_ids:
        if rule in standard_rules:
            ...
            engine_logger.error(f"The rule specified '{rule}' is not in the standard {standard} and version {version}")
    rules = []
...
def load_all_rules_for_standard(rules_data, standard, version, substandard, standard_dict):
    rules = []
    log_message = f"No rules specified. Running all rules for {standard} version {version}"
    if substandard:
...
def load_all_rules(rules_data):
    rules = []
    core_ids = set()
    engine_logger.info("No rules, standard, or version specified. Running all local rules.")
...
def load_rules_from_cache(args) -> List[dict]:
    ...
    try:
        ...
        engine_logger.error(f"Rules file or dictionary not found: check {rules_file}, {cdisc_file}, {standard_dict}")
        return []
    except Exception as e:
...
def load_rules_from_local(args) -> List[dict]:
    ...
    if args.rules:
        keys = set(get_rules_cache_key(args.standard, args.version.replace(".", "-"), rule) for rule in args.rules)
    else:
        engine_logger.info("No rules specified with -r rules flag. " "Validating with all rules in local directory")
        keys = None
    ...
    if missing_keys:
        ...
        engine_logger.error(f"Specified rules not found in the local directory: {missing_keys_str}")
    return rules
...
def process_rule(rule, args, rule_data, rules, keys):
    ...
    rule_identifier = get_rules_cache_key(args.standard, args.version.replace(".", "-"), core_id)
    ...
    if args.standard and args.version:
        if not rule_matches_standard_version(rule, args.standard, args.version, args.substandard):
            substandard_msg = f" with substandard '{args.substandard}'" if args.substandard else ""
            engine_logger.info(
    ...
    if keys is None or rule_identifier in keys:
        ...
        engine_logger.info(f"Rule {core_id} not specified with " "-r rule flag and in local directory. Skipping...")
