...
from cdisc_rules_engine.DatasetOperations.helpers import get_code_hierarchies
...
def test_valid_whodrug_references(
    ...
    result = operations.get_service(
        "whodrug_code_hierarchy", operation_params, invalid_df, cache, data_service
    )
...
def test_valid_whodrug_references_dask(
    ...
    assert operation_params.operation_id in result
    for res, exp in zip(
        result[operation_params.operation_id], pd.Series([True, True, False, False])
    ):
        assert res == exp
...
def test_get_code_hierarchies(tmp_path, operation_params: OperationParams):
    ...
    valid_hierarchies = get_code_hierarchies(terms)
