...
def test_element_sharing_operators(data, dataset_type, operator, expected_result):
    ...
    result = getattr(dataframe_type, operator)({"target": "target", "comparator": "comparator"})
...
def test_element_sharing_operators_cases():
    ...
    assert dataframe_type.shares_at_least_one_element_with({"target": "target", "comparator": "comparator"})

    # Changed expectation to True
    assert dataframe_type.shares_exactly_one_element_with({"target": "target", "comparator": "comparator"})

    assert not dataframe_type.shares_no_elements_with({"target": "target", "comparator": "comparator"})
