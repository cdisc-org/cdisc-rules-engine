...
class RuleTester:
    ...
    def validate(self, rule) -> dict:
        ...
        for dataset_metadata in self.datasets:
            if dataset_metadata.domain in validated_domains:
                continue  # handling split datasets
            rule["conditions"] = ConditionCompositeFactory.get_condition_composite(
                rule["conditions"]
            )
            results[dataset_metadata.domain] = self.engine.validate_single_rule(
                rule,
                f"/{dataset_metadata.filename}",
                self.datasets,
                dataset_metadata,
            )
            validated_domains.add(dataset_metadata.domain)
        return results
