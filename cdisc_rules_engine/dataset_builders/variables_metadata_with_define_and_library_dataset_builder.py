...
class VariablesMetadataWithDefineAndLibraryDatasetBuilder(BaseDatasetBuilder):
    def build(self):
        ...
        define_vlm_dataset = self.dataset_implementation.from_records(
            define_vlm_records
        )
        ...
        if has_vlm:
            vlm_summary = define_vlm_df.groupby(
                "define_variable_name", as_index=False
            ).agg(
        ...
        vlm_summary = vlm_summary.rename(
            columns={"define_variable_name": "variable_name"}
        )
        ...
        final_dataframe["define_vlm_present"] = final_dataframe[
            "define_vlm_present"
        ].fillna(False)
        final_dataframe["define_vlm_item_count"] = (
            final_dataframe["define_vlm_item_count"].fillna(0).astype(int)
        )
        final_dataframe["define_vlm_ccodes"] = final_dataframe[
            "define_vlm_ccodes"
        ...
        final_dataframe["define_vlm_ccode_matches_library_any"] = final_dataframe.apply(
            lambda row: (
                row["library_variable_ccode"] in row["define_vlm_ccodes"]
                if row["define_vlm_ccodes"]
                else False
            ),
            axis=1,
        )
        final_dataframe["define_vlm_ccode_matches_library_all"] = final_dataframe.apply(
            lambda row: (
                ...
                and all(
                    c == row["library_variable_ccode"] for c in row["define_vlm_ccodes"]
                )
            ),
            axis=1,
        )
